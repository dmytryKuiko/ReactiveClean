...
class SearchModelDisplayableAdapter(
    ...
    override fun onCreateViewHolder(parent: ViewGroup): RecyclerView.ViewHolder {
        val view = LayoutInflater.from(parent.context).inflate(R.layout.row_search_displayable, parent, false)
        return SearchViewHolder(view, callback)
    }

    override fun onBindViewHolder(items: MutableList<SearchDisplayable>, position: Int, holder: RecyclerView.ViewHolder, payloads: MutableList<Any>) {
        val item = items[position] as? SearchDisplayable.Search ?: throw ClassCastException("SearchDisplayable is not a Search")
        val contentDisplayableHolder = holder as? SearchViewHolder ?: throw ClassCastException("not a SearchViewHolder")
        contentDisplayableHolder.bind(item)
    }

    inner class SearchViewHolder(view: View, callback: (SearchDisplayable.Search) -> Unit) : RecyclerView.ViewHolder(view) {
